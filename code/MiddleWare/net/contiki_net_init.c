...
void contiki_set_link_addr(bool_t bIsTest, uint8 *macaddr)
{
  ...
  tLinkAddr.u8[7] = get_moteid() & 0xff; 
  tLinkAddr.u8[6] = (get_moteid()>>8) & 0xff; 

  PRINTF("%d,%d",tLinkAddr.u8[0],tLinkAddr.u8[1]);
...
#if NETSTACK_CONF_WITH_IPV6
  memcpy(&uip_lladdr.addr, &tLinkAddr, sizeof(uip_lladdr.addr));     //把tLinkAddr里的内容复制到uip_lladdr.addr中
#endif
